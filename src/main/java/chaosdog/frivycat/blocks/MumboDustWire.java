...
public class MumboDustWire extends Block {
    ...
    public MumboDustWire(Properties properties, BlockState sideBaseState) {
        super(properties);
        this.sideBaseState = sideBaseState;
    }
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
        if (facing == Direction.DOWN) {
            ...
            return redstoneside.isValidSide() == stateIn.get(FACING_PROPERTY_MAP.get(facing)).isValidSide() && !areAllSidesValid(stateIn) ? stateIn.with(FACING_PROPERTY_MAP.get(facing), redstoneside) : this.getUpdatedState(worldIn, this.sideBaseState.with(POWER, stateIn.get(POWER)).with(FACING_PROPERTY_MAP.get(facing), redstoneside), currentPos);
        }
    }
    ...
    public int getWeakPower(BlockState blockState, IBlockReader blockAccess, BlockPos pos, Direction side) {
        if (this.canProvidePower && side != Direction.DOWN) {
            ...
            if (i == 0) {
                ...
                return side != Direction.UP && !this.getUpdatedState(blockAccess, blockState, pos).get(FACING_PROPERTY_MAP.get(side.getOpposite())).isValidSide() ? 0 : i;
            }
        } else {
            return 0;
        }
